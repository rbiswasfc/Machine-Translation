...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        """ 
        return [[[self.char2id['{']] + [self.char2id[w] for w in word] + [self.char2id['}']] for word in sent] for sent in sents]
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        sent2id = self.words2charindices(sents)
        sent_padded = pad_sents_char(sent2id, self.char2id['<pad>']) #(batch,max_len,max_char)
        sent_padded = torch.tensor(sent_padded, dtype=torch.long, device=device)
        
        return torch.transpose(sent_padded,0,1) #
        ### END YOUR CODE
